(() => {
  ...
  const CHAPTER_DEFS = [
    ['Introducción', 'Hola. Bienvenido al módulo dos'],
    ['Qué es IA generativa', 'Empecemos por la definición central'],
    ['Formatos que puede producir', 'Ahora miremos los formatos'],
    ['Cómo funciona', 'Pasemos ahora a la pregunta más interesante'],
    ['Modelos de lenguaje y LLM', 'Ahora llegamos a los modelos de lenguaje'],
    ['Oportunidades y límites', 'Pasemos al terreno organizacional'],
    ['Síntesis final', 'Podemos cerrar este módulo']
  ];

  const savedRate = Number(localStorage.getItem('m2.rate'));
  ...
  function populateVoices() {
    ...
    const previousName = localStorage.getItem('m2.voice');
    el.voice.innerHTML = '';
  ...
  async function sharePage() {
    const data = { title: 'M2 - IA Generativa', text: 'Clase M2 - IA Generativa', url: location.href };
    if (navigator.share) {
  ...
  el.voice.addEventListener('change', () => {
    const voice = selectedVoice();
    if (voice) localStorage.setItem('m2.voice', voice.name);
    updateVoiceBadge();
  ...
  el.rate.addEventListener('change', () => {
    localStorage.setItem('m2.rate', el.rate.value);
    if (state.isPlaying && !state.isPaused) startFromCurrent({ afterCancel: true });
  });
